...
<h1>Listagem de Certificação de Reprodutores</h1>
...
@include('admin.certificacao_reprodutores.images.create')
@include('admin.certificacao_reprodutores.images.imagem')
            ...
            <tbody>
                @foreach($certificacao_reprodutores as $certificacao)
                    ...
                    <tr>
                        ...
                        <td>{{ empty($certificacao->tanque_origem->sigla) ? '' : $certificacao->tanque_origem->sigla  }}</td>
                        <td>{{ empty($certificacao->tanque_maturacao->sigla)  ? '' : $certificacao->tanque_maturacao->sigla }}</td>
                        <td>
                            @php
                                //dd(asset('storage/app/public/'.$certificacao->imagem_certificacao));
                            @endphp
                            @if (! $certificacao->imagem_certificacao != "")
                                <span class="label label-danger" title="Não existe imagem cadastrada" style="text-decoration: none"><a  href="#">Imagem</a></span>
                            @else 
                                <span class="label label-success" title="Clique para baixar" style="cursor:pointer;"><a a href="#" data-toggle="modal" data-target="#image_show_modal" data-denominacao="{{$certificacao->id}}" onclick="onClickImageModal('{{ asset('storage').'/'.$certificacao->imagem_certificacao }}')">Imagem</span>
                            @endif  
                        </td>
                        <td>
                            
                            @if ($certificacao->status() == '1')
                                <span class="label label-danger">{{ $certificacao->situacao() }}</span>
                            @else 
                                <span class="label label-success">{{ $certificacao->situacao() }}</span>
                            @endif    
                        </td>
                        <td>
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a href="#" data-toggle="modal" data-target="#imagem_analise_modal" data-denominacao="{{$certificacao->id}}" onclick="onClickFormModal({{$certificacao->id}})"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i>Imagem da Análise</a></li>
                                </ul>
                            </div>
