...
<div class="box">
    ...
    <div class="box-body">
        @php
            $numero_certificacao  = ! empty(session('numero_certificacao'))   ? session('numero_certificacao')   : old('numero_certificacao');
            $plantel              = ! empty(session('plantel'))               ? session('plantel')               : old('plantel');
            $familia              = ! empty(session('familia'))               ? session('familia')               : old('familia');
            $data_estresse        = ! empty(session('data_estresse'))         ? session('data_estresse')         : old('data_estresse');
            $data_coleta          = ! empty(session('data_coleta'))           ? session('data_coleta')           : old('data_coleta');
            $data_certificacao    = ! empty(session('data_certificacao'))     ? session('data_certificacao')     : old('data_certificacao');
            $tanque_origem_id     = ! empty(session('tanque_origem_id'))      ? session('tanque_origem_id')      : old('tanque_origem_id');
            $tanque_maturacao_id  = ! empty(session('tanque_maturacao_id'))   ? session('tanque_maturacao_id')   : old('tanque_maturacao_id');
        @endphp
        <form action="{{ route('admin.certificacao_reprodutores.to_store') }}" method="POST">
