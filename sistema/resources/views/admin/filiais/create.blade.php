...
<div class="box">
    ...
    <div class="box-body">
        @php
            ...
            $cidade   = !empty(session('cidade'))   ? session('cidade')   : old('cidade');
            $endereco = !empty(session('endereco')) ? session('endereco') : old('endereco');
            $cnpj     = !empty(session('cnpj'))     ? session('cnpj')     : old('cnpj');
        @endphp
        <form action="{{ route('admin.filiais.to_store') }}" method="post" enctype="multipart/form-data">
            {!! csrf_field() !!}
