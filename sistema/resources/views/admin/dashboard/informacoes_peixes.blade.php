...
<h1>Tanque {{ $tanque->sigla }}
    @if(! is_null($lote))
        <small><b>Lote {{ $lote->codigo }}</b></small>
    @endif
</h1>
<ol class="breadcrumb">
...
<div class="box box-primary">
    ...
    <div class="box-body">
        ...
        @if (! is_null($lote))
            <div class="row">
                <div class="col-xs-2"><b>Lote:</b> {{ $lote->codigo }}</div>
                <div class="col-xs-2"><b>Início:</b> {{ $lote->data_inicio() }}</div>
                <div class="col-xs-2"><b>Atividade:</b> {{ $lote->dias_atividade() . ' Dias' }}</div>
            </div>
            <hr>
        @endif
                                    ...
                                    <tbody>
                                        @foreach ($saidas_povoamento as $saida_povoamento)
                                            <tr style="{{ $saida_povoamento->estoque_saida->tipo_destino == 7 ? 'background-color:#ff000036; color:#ffffff;' : '' }}">
                                                ...
                                                <td>{{ $saida_povoamento->lote->lote_fabricacao }}</td>
                    ...
                    <div class="box-body">
                        @php
                            ...
                            if (! is_null($lote)) {
                                ...
                                $data_referencia = $lote->data_inicio;

                                if (! is_null($lote->data_fim)) {
                                    $data_referencia = $lote->data_fim;
                                }
