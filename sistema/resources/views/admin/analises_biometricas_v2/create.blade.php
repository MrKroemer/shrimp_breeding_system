...
<div class="box">
    ...
    <div class="box-body">
        ...
        @endphp
        <form action="{{ route('admin.analises_biometricas_v2.to_store') }}" method="post" enctype="multipart/form-data">
            {!! csrf_field() !!}
            <div class="row">
                <div class="col-xs-6">
            ...
            <div class="row">
                <div class="col-xs-4" id="total_amostras">
                    ...
                    <input type="number" step="any" name="total_amostras" placeholder="Total de Amostras" class="form-control" value="{{ $sobrevivencia }}">
                </div>
                <div class="col-xs-4">
                    <label for="peso_total">Peso Total:</label>
                    <input type="number" step="any" name="peso_total" placeholder="Peso Total" class="form-control" value="{{ $sobrevivencia }}">
                </div>
                <div id="sobrevivencia" class="col-xs-4">
...
@foreach($ciclos as $ciclo)
    <input type="hidden" name="{{ $ciclo->id }}" id="{{ $ciclo->id }}" value="{{ $ciclo->tipo }}" />
@endforeach
@endsection