...
class Preparacoes extends Model
{
    const CREATED_AT = 'criado_em';
    const UPDATED_AT = 'alterado_em';

    protected $table = 'preparacoes';
    ...
    public function abas_inicio(String $format = 'd/m/Y')
    {
        if (! $this->abas_inicio) {
            return $this->abas_inicio;
        }

        return Carbon::parse($this->abas_inicio)->format($format);
    }

    public function abas_fim(String $format = 'd/m/Y')
    {
        if (! $this->abas_fim) {
            return $this->abas_fim;
        }

        return Carbon::parse($this->abas_fim)->format($format);
    }
