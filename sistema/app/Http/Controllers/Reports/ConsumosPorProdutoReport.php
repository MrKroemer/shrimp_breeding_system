...
class ConsumosPorProdutoReport extends mPDF
{
    ...
    {   
        ...
        foreach ($estoque_saidas as $estoque_saida) {
            ...
            foreach ($consumos as $consumo) {
                ...
                if (! is_null($consumo['saida']->ciclo)) {
                    $this->documentBody .= '<td>' . $consumo['saida']->tanque->sigla . ' (Nº'. $consumo['saida']->ciclo->numero . ')</td>';
                } else {
                    $this->documentBody .= '<td>' . $consumo['saida']->tanque->sigla . '</td>';
                }
                
                /*$consumo['saida']->valor_total*/ $total =  $consumo['saida']->quantidade * $consumo['saida']->valor_unitario;

                $this->documentBody .= '<td>' . DataPolisher::numberFormat($consumo['saida']->quantidade, 4) . ' ' . $consumo['saida']->produto->unidade_saida->sigla . '</td>';
                $this->documentBody .= '<td>R$ ' . DataPolisher::numberFormat($consumo['saida']->valor_unitario) . '</td>';
                $this->documentBody .= '<td>R$ ' . /*DataPolisher::numberFormat($consumo['saida']->valor_total)*/ DataPolisher::numberFormat($total) . '</td>';
                $this->documentBody .= '<td>' . $consumo['saida']->data_movimento() . '</td>';

                $this->documentBody .= '</tr>';

                $qtd_total   += $consumo['saida']->quantidade;
                $custo_total += $total; //$consumo['saida']->valor_total;

            }

            $this->documentBody .= '<tr>';
            $this->documentBody .= '<td style="border-top: 1px solid #000; background-color: #e6e6e6; font-weight: bold;">TOTAIS:</td>';
            $this->documentBody .= '<td style="border-top: 1px solid #000; background-color: #e6e6e6; font-weight: bold;">' . DataPolisher::numberFormat($qtd_total, 4) . ' ' . $consumo['saida']->produto->unidade_saida->sigla . '</td>';
